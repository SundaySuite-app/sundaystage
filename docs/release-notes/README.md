...
Én fil per tagg: `v0.8.0-beta.1.md`. Fila er teksten operatøren leser i
...
var en fast setning i `release.yml`, fikk hver eneste utgivelse den samme
innholdsløse teksten: v0.6.0-beta.1, v0.7.0 og v0.8.0-beta.1 bærer alle
«See the assets below to download and install…».

Det gjorde konkret skade ved v0.8.0-beta.1, som flyttet blackout fra Escape til
⇧B. Operatøren som fikk oppdateringsvarselet fikk ingen anelse om at
nødstopptasten hadde byttet plass.
...
1. Bump versjonen i `package.json` **og** `src-tauri/tauri.conf.json`.
2. Lag `docs/release-notes/v<versjon>.md`.
3. `pnpm run notes:check` — samme vakt som CI kjører.

Skriv til en frivillig som står ved skjermen ti minutter før gudstjenesten, ikke
til en utvikler. Det viktigste først: har noe flyttet seg, eller kan noe
overraske midt i en gudstjeneste, skal det stå i første setning.
...
- **Versjonene må være i takt.** `package.json` og `src-tauri/tauri.conf.json`
  må si det samme, ellers er det ikke sikkert notatet hører til taggen som
  bygges.

Vakten er `tests/integration/releaseNotes.test.ts`, og den kjører i `check`-jobben
på hver PR. Den stopper altså slippet før noen har begynt å bygge — ikke femten
minutter inn i et bygg.
...
`latest.json` er allerede lastet opp med notatet herfra, og promoteringsflyten
sammenlikner manifestet — ikke utgivelsesteksten.
