...
| Telemetry ingest (E5)       | With `SUNDAYSTAGE_TELEMETRY_URL` + `_WRITE_KEY` baked in and consent granted: a queued payload POSTs to `/v1/apps/sundaystage/ingest` and is accepted (202); an unknown field is refused 4xx and dropped WITHOUT a retry; a 429 honours `Retry-After`. | `src-tauri/.../telemetry/http_sender.rs`                            | NETWORK-UNVERIFIED |
| Telemetry deletion (E5)     | "Delete my data" → `DELETE /v1/apps/sundaystage/install/:id` succeeds, and succeeds again for an id the endpoint has never seen. Must work with consent OFF — that is the case it exists for.                                                          | `src-tauri/.../telemetry/sender.rs` `drain_deletions`               | NETWORK-UNVERIFIED |
