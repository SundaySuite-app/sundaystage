 ...
 * brukeren. v0.6.0-beta.1, v0.7.0 og v0.8.0-beta.1 bærer alle samme
 * innholdsløse engelske setning.
 *
 * Det som gjorde skade var v0.8.0-beta.1: den flyttet blackout fra Escape til
 * ⇧B, midt i det operatøren bruker som nødstopp, og oppdateringsvarselet sa
 * ingenting om det.
 *
 * Vakten ligger her — i `check`-jobben, på PR — og ikke i `release.yml`, fordi
 * det er her noen kan gjøre noe med den. Feiler den i et bygg, er slippet
...
describe("releasenotat", () => {
  ...
  it("er kort nok til å bo i et manifest som hentes ved hver sjekk", () => {
    for (const tag of knownTags()) {
      ...
      expect(
        note.ok,
        `${tag}: ${note.ok ? "" : note.problems.join("; ")}`,
      ).toBe(true);
  ...
  it("nevner ⇧B i notatet for versjonen som flyttet blackout", () => {
    // Denne testen er ikke generell, og skal ikke være det: den er en knagg på
    // den ene endringen som gjorde skade, slik at notatet ikke kan slankes bort
    // uten at noen tar stilling til det.
