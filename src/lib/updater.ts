 ...
 *
 * Up to v0.4.0 this called `check()` from `@tauri-apps/plugin-updater`, which
 * always uses the endpoints baked into `tauri.conf.json` — its `CheckOptions`
 * has no `endpoints` field. Since the operator can choose an update ring
 ...
 * install underneath are still the updater plugin's own code.
 *
