 ...
 * Så lenge `releaseBody` var en fast engelsk setning i `release.yml`, var
 * dialogen garantert innholdsløs: v0.6.0-beta.1, v0.7.0 og v0.8.0-beta.1 bærer
 * alle nøyaktig samme «See the assets below…». v0.8.0-beta.1 flyttet blackout
 * fra Escape til ⇧B — en vane-endring midt i en gudstjeneste — og operatøren
 * som fikk oppdateringsvarselet så ingenting om det.
 *
 ...
 *
 * - **Ren tekst, ikke markdown.** Notatet vises i en liten boks nede i hjørnet
 *   av appen, ikke i en markdown-renderer. `**fet**` og tabellrader blir stående
 *   som tegn på skjermen. Regelen tvinger fram den skrivemåten dialogen
 *   fortjener uansett: korte setninger til en frivillig som står ved skjermen.
 ...
 *   verre enn ingen fil: vakten under blir grønn og feilen blir usynlig igjen.
 *
 * Vil du ha den lange versjonen på GitHub-siden, rediger utgivelsesteksten der
 * etterpå. Det er trygt: `latest.json` er allerede lastet opp, og
 * promoteringsflyten sammenlikner manifestet — ikke utgivelsesteksten.
 *
 * ## Bruk
 *
 *   node scripts/release-notes.mjs --check          # vakt: sjekker alle notater
 *                                                   # + at versjonen har sitt
 *   node scripts/release-notes.mjs --emit v1.2.3    # skriver GITHUB_OUTPUT-blokk
 *
 * Modulen importeres også av `tests/integration/releaseNotes.test.ts`, som er
 * vakten som faktisk stopper en PR. `--check` finnes for å kunne kjøres for
 * hånd; CI-vakten er testen.
 */
...
/** Der notatene bor. Én fil per tagg: `v0.8.0-beta.1.md`. */
export const NOTES_DIR = join(REPO_ROOT, "docs", "release-notes");

/**
 * Taket for ett notat, i BYTES — ikke tegn. `«»`, `⇧` og `⌘` er flere bytes
 * hver, og det er bytes `latest.json` bærer over nettet.
 *
 * 1000 bytes er rundt 12 linjer i dialogboksen. Det er rikelig til «dette er
 * nytt, og dette må du vite før søndag», og altfor lite til en utviklerlogg —
 * som er hele poenget med et tak.
 */
...
/** Versjonen appen bygges som, fra `package.json`. */
...
export function tauriVersion() {
  return readJson(join(REPO_ROOT, "src-tauri", "tauri.conf.json")).version;
}
...
const PLAIN_TEXT_RULES = [
  ...
  [/`/, "backticks — de blir stående som tegn; skriv ⇧B, ikke `⇧B`"],
...
/**
 * Hele vakten: versjonene er i takt, versjonen har sitt notat, og ingen av de
 * andre notatene har råtnet.
 */
export function checkAll() {
  ...
  if (pkg !== tauri) {
    problems.push(
      `package.json er ${pkg} og src-tauri/tauri.conf.json er ${tauri} — taggen bygget lages fra må være den notatet er skrevet for`,
    );
  }
...
function main(argv) {
  ...
  if (mode === "--check") {
    ...
    console.log(
      `OK — ${result.tag} har notat, og ${knownTags().length} notat(er) er gyldige.`,
    );
    return;
  }
